...
#include <termios.h> //Terminalkontrollfunktionen (Puffer, Verz�gerung, Echo, etc.)
#include <unistd.h>  //Stellt unter anderem sleep und usleep bereit
#include <fcntl.h>	 //Unix Systemcalls (Filedescriptor) Lese-/Schreibfunktionalit�t
#include <chrono>	 //Zeitumrechnungsfunktionen und exakte Zeitnahme

//Funktion pr�ft auf Tastatureingabe f�r die Dauer des Intervalls in ms
char SimpleInput::getSteuerung(long interval)
{
	...
	oldtio.c_lflag &= ~(ECHO);				// Echo f�r Eingaben dauerhaft auf false setzen.
	tio = oldtio;
	tio.c_lflag &= ~(ICANON | ECHO); 		// Verz�gerungsfreies Einlesen ohne Ausgabe auf der Konsole
	...
	while (duration.count() < interval)
	{
		...
		//Ermitteln der Restzeit f�r das Warten auf Tastatureingaben
		duration = std::chrono::duration_cast<std::chrono::milliseconds>
			(std::chrono::high_resolution_clock::now() - startzeit);
	}
	// Zur�cksetzen der Terminaleinstellung
	...
	//Mainthread pausiert f�r die Dauer der Restzeit
	usleep(dauer);
	return c;
}
